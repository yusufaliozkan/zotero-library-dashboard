...
try:
    ...
    if choice == 'Group library (use group library link)':
        ...
        library_id = st.text_input('Personal library ID: (Find it [here](https://www.zotero.org/settings/keys). Check the "Your userID for use in API calls" section.)')
        api_key = st.text_input("Zotero API key: (Find it [here](https://www.zotero.org/settings/keys/new). Create a new private key if you don't have an existing one.) ")
        library_type = 'user'
    ...
    with st.spinner('Creating dashboard. This may take a while if the library contains more items.'):
        if  display:
            ...
            with col1:
                ...
                wordcloud = WordCloud(stopwords=stopword, width=1500, height=750, background_color='white', collocations=False, colormap='magma', font_path='path_to_your_font_file.ttf').generate(wordcloud_texts_str)
